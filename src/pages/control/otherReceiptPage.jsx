...
const normaliseBanks = (raw) => {
  ...
  const fetchReceipts = async () => {
    try {
      ...
      const filtered = data
        .filter((i) =>
          i.transactionType === "OtherReceipt"
        )
  ...
  const fetchOutstandingInvoices = async (memberId) => {
    const res = await axios.get(
      `${import.meta.env.VITE_BACKEND_URL}/api/ledger-transaction/due/${memberId}`,
      { headers }
    );
    ...
    try {
      ...
      if (form.paymentMethod === "Card") {
        if (form.cardType === "Visa") {
          ...
          accountId = "303-003";
          accountName = "American Express (Amex)";
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
        <div className="space-y-1">
          ...
          <p className="text-sm text-gray-500 mt-1">
            Create and manage incoming payment receipts for others.
          </p>         
        </div>
          ...
          {filteredMembers.map((c) => (
            ...
            <button
              ...
              onClick={() => {
                ...
                setForm((prev) => ({
                  ...prev,
                  memberId: c.accountId,
                  memberName: c.accountName,
