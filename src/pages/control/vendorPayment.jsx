...
const normaliseBanks = (raw) => {
  ...
  /* ------------------ fetch accounts + bank data ------------- */
    useEffect(() => {
      ...
      try {
          const [vendorsRes, accRes, banksRes, branchesRes] = await Promise.all([
  ...
  const fetchOutstandingInvoices = async (vendorId) => {
    const res = await axios.get(
      `${import.meta.env.VITE_BACKEND_URL}/api/vendor-transaction/due/${vendorId}`,
      { headers }
    );
            ...
            <button
              ...
              onClick={() => {
                ...
                setForm((prev) => ({
                  ...
                  totalOutstanding: c.vendorDueAmount || 0,
                }));
